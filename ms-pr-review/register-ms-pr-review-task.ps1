...
.EXAMPLE
    pwsh -File C:\neldevsrc\Github\ExternalAPITaskSchedulerPrompt\ms-pr-review\register-ms-pr-review-task.ps1
.EXAMPLE
    pwsh -File C:\neldevsrc\Github\ExternalAPITaskSchedulerPrompt\ms-pr-review\register-ms-pr-review-task.ps1 -RunNow      # register, then trigger once now
.EXAMPLE
    pwsh -File C:\neldevsrc\Github\ExternalAPITaskSchedulerPrompt\ms-pr-review\register-ms-pr-review-task.ps1 -Model sonnet
.EXAMPLE
    pwsh -File C:\neldevsrc\Github\ExternalAPITaskSchedulerPrompt\ms-pr-review\register-ms-pr-review-task.ps1 -Unregister
#>
param(
...
Write-Host "Logs:    C:\neldevsrc\Github\ExternalAPITaskSchedulerPrompt\ms-pr-review\logs\$TaskName\"

if ($RunNow) {
    Write-Host "`nTriggering a run now..." -ForegroundColor Cyan
    Start-ScheduledTask -TaskName $TaskName
    Write-Host "Started. Tail the newest log:" -ForegroundColor Cyan
    Write-Host "  Get-ChildItem `"C:\neldevsrc\Github\ExternalAPITaskSchedulerPrompt\ms-pr-review\logs\$TaskName`" | Sort LastWriteTime -Desc | Select -First 1 | %{ Get-Content -Wait `$_.FullName }"
}
