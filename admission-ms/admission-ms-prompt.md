...
- **Scope every `Grep`.** `{{REPO_ROOT}}` is a 40+ service monorepo; an unscoped repo-wide `Grep` hits the 20-second ripgrep cap and returns nothing (`Ripgrep search timed out after 20 seconds` — it cost a retry on 2026-08-11). Always pass `path` (e.g. `src/Services.Admissions`, `src/Services.Admissions/Admissions.Accessors/EFModel`) and/or `glob` (e.g. `*.cs`), and narrow the pattern before widening the path.
...
- **ViewModels** — the **complete DTO field list with types and nullability**. This is the DTO contract: include **every listed property, exactly as named/typed**, and nothing extra. If the schema truly cannot yield a listed field, do NOT silently drop it: keep it in the DTO, leave a `// TODO(AB#<id>)`, and call it out in the summary.
  - **⚠️ ViewModels NAMES the field; the SCHEMA SIZES it — mirror the EF entity's CLR type for every scalar.** ViewModels is authoritative for *which* properties exist, their names, and their nullability. It is hand-written, though, and routinely states a **wider integer than the column actually holds** — so it is **not** authoritative for numeric width. Before typing any numeric DTO property, read the real column type from the story's `sis-sql-schema` DDL (and from the `EFModel/` entity, which already mirrors it) and use the matching CLR type — `tinyint → byte`, `smallint → short`, `int → int`, `bigint → long`, `bit → bool` — keeping the nullability ViewModels states. **A narrower→wider projection (`byte? → short?`) is an implicit, lossless conversion, so it compiles and every test passes** — which is exactly why this slips past the build and surfaces only in review.
    - **Real miss (PR #3273, `OAFormSchool`):** ViewModels said `SchoolSortOrder [short]`; `dbo.OAFormSchool.SchoolSortOrder` is `tinyint` and the EF entity is `byte?`. The generated DTO shipped `short?` and review flagged it against the architecture doc's type-match rule (**§13, ADO ↔ DTO ↔ EF**) — *"ADO's `short` is the imprecise value here."* Every other tinyint-backed DTO in the service already mirrors its entity (`OAAddressFieldDto.NumPreviousSchools`, `OAAddressParentPreferencesDto.OneOrTwo`, `OAImportStatusTypeDto.Id` — all `byte`), so the generated `short?` was the outlier, not the convention.
    - **`ConfigSchoolId` is the standing exception:** it stays `short` because it mirrors `dbo.ConfigSchool.ConfigSchoolID` (`smallint`) reached through the nav chain — not a column on the entity's own table. Same for the `(short)0` fallback in the projection.
    - **The chosen type must land in EVERY generated tier or they silently drift:** the DTO, `ApiTests/Features/<Feature>/Shared/<Feature>DtoTm.cs`, the faker's cast (`(byte)f.Random.Number(1, 200)`, `allowNulls ? (byte?)null : …`), and any unit-test assertion cast (`Should().Be((byte)5)`). A DTO widened without its `Tm` is a contract split that no test will catch. **`*.verified.txt` snapshots are unaffected** — Verify serializes the value, not the CLR type — so a width fix needs no snapshot re-record.
    - **One behavioral consequence worth knowing (it is a feature, not a regression):** with the exact type, an out-of-range Sieve filter (`SchoolSortOrder==300` against a `byte?`) fails conversion instead of quietly matching nothing.
    - **Log the resolution** for every numeric property where ADO and the schema disagree — one line each, in the per-story analysis:
      ```
      DTO-TYPE CHECK: <Feature>.<Prop> — ADO <type> / SQL <type> / EF <type> → DTO <chosen type>
      ```
  - **⚠️ DTO property names are .NET PascalCase — NEVER raw DB column casing. Mirror the EF entity's property name, not the SQL column name.** FACTS SIS columns are largely lowercase-run-together (`memberid`, `questionid`, `requestid`, `appstatus`, `sortorder`), and ViewModels often echoes that casing. The `EFModel/` entity has **already** done the translation — `MemberId`, `QuestionId`, `RequestId`, `AppStatus`, `SortOrder` — via `[Column("memberid")]`. **Take the DTO property name from the EF entity, not from the column or from ViewModels' casing.** Compound words split on the word boundary: `appstatus → AppStatus`, `sortorder → SortOrder`, and an `id` suffix is always `Id` — `memberid → MemberId`, never `Memberid`.
    - **Why this slips through:** the projection `Memberid = inquiryBySchool.MemberId` compiles perfectly, every test passes, and the snapshot happily records `Memberid` — nothing fails. It surfaces only in review, as a public API contract that has to be broken to fix.
    - **Real miss (PR #3414):** the generated `OAInquiryBySchoolDto`, `OARequestInfoAnswerDto` and `OAStatusDto` shipped `Memberid`, `Questionid`, `Requestid`, `Appstatus` and `Sortorder` while the EF entities beside them already read `MemberId`/`QuestionId`/`RequestId`/`AppStatus`/`SortOrder`. Reviewer filed five separate one-line suggestions. Note the blast radius of the late fix: the property, the projection, any `OrderBy`, the `DtoTm`, every Sieve `Filters=`/`Sorts=` string in the tests, **and the `sortParam=<Name>` embedded in Verify snapshot FILE NAMES** — 55 files for five properties.
    - **Check the whole DTO before you write it**, and log one line per property whose column casing differs from its DTO name:
      ```
      DTO-NAME CHECK: <Feature>.<Prop> — SQL <column> / EF <property> → DTO <chosen name>
      ```
    - Already-merged features carry this defect too (`OARequestInfoStudentDto.Requestid`, `OARequestInfoQuestionDto.Sortorder`/`Memberid`). **Do not "fix" them in your run** — they are a separate breaking-change sweep. Match the EF entity for the DTOs *you* generate, and mention the pre-existing ones in the summary.
  ...
  - **⚠️ OMITTING `ConfigSchoolId` IS A LAST RESORT — the bar is high, and a real run cleared it too easily.** Omission is correct ONLY when the full ladder below (all 8 steps, **including the junction-table search and the ADO-test-suite cross-check**) proves there is no path. It is **not** enough that the entity lacks an FK, or that the first hop is one-to-many — that is exactly the situation a junction table resolves. When omission IS proven: do **not** emit `ConfigSchoolId = (short)0` or any constant, and do **not** leave a filterable `ConfigSchoolId` property on the DTO — a Sieve-filterable field hard-wired to `0` silently breaks every `ConfigSchoolId==…` filter. Instead **remove the property, its projection line, and any Sieve attribute**, make the omission LOUD (`// NOTE(AB#<id>): ConfigSchoolId omitted — …` in the DTO + prominently in the run summary), and mark the story `scaffold` pending a design decision. **This overrides the general "keep it + TODO" rule above for `ConfigSchoolId` specifically** — for a filterable tenancy field, a truthful omission beats a misleading placeholder.
    ...
    - **🛑 THE ONE EXCEPTION — a GLOBAL LOOKUP / REFERENCE table has no school, and forcing `ConfigSchoolId` onto it is a BUG, not compliance.** This is **not** the last-resort omission above and does **not** make the story `scaffold` — it is the correct, complete implementation. Decide it with **step 0 of the ladder below, before you walk any FKs.** The test is the **direction of the join**: a real tenancy path runs **outward from the entity** through its own FKs to the school it belongs to (one school per row). A lookup has no such FK — the only way to reach a school is to join **inward**, from the rows that *reference* the entity, and then the answer is "every school that happens to use this value". That is usage data, not a property of the row.
      - **Why it is a bug and not just noise:** synthesizing the field forces `.Distinct()` over a join to consumer tables, which **fans the response out to one row per (lookup value, school)** and destroys the lookup contract — the PK is no longer unique in the response, so `Filters=Id==3` returns N rows instead of one. It also scans the consumer tables to return a handful of rows.
      - **Real miss (PR #3414, `dbo.OAStatus` + `ref.OAReportSettingsType`):** both are global lookups (`OAStatus` is `appstatusid/appstatus/sortorder/SIS/EAdmissions` — no `SchoolCode`, no `ConfigSchoolID`; `ref.OAReportSettingsType` is 5 rows of `Id/Description`). To satisfy the convention the run joined `OAStatus → OAStudent → OA → ConfigSchool` and `OAReportSettingsType → OAReportSettings → OAInquiryBySchool → ConfigSchool`, three chained outer joins each, then `.Distinct()`. The committed `BasicRetrieval` snapshot shipped **8 rows for 6 statuses, with `Id: 20000` and `Id: 20001` each appearing twice** — and nobody caught it, because `ConfigSchoolId` was in the scrub list, so the duplicate rows looked identical in the snapshot. Reviewer: *"OAStatus and ref.OAReportSettingsType are global lookup tables … Id is the primary key but it is no longer unique in the response … Can we drop ConfigSchoolId from these two DTOs and return the lookup as-is?"* The fix removed the field, both handlers collapsed to the plain `_context.X.AsNoTracking().AsQueryable().Select(Dto.Projection)` exemplar shape, and 9 of 18 ADO cases per feature had to be retired.
      - **When you conclude "global lookup":** drop the property + projection + Sieve attribute (as with any omission), keep the handler at the plain exemplar shape, and make it LOUD — a `NOTE(AB#<id>)` on the DTO **and** on the test class **and** in the run summary, each stating *which* table it is a lookup for and *why* an inward join would break the PK. Then **assert the contract you just protected**: `Filters=Id==<pk>` must return exactly one row, and `BasicRetrieval` must assert `Results.Select(x => x.Id).Should().OnlyHaveUniqueItems()`. Ship the story `full`.
      - **The ADO-suite cross-check (step 6) does NOT override this.** A suite full of `ConfigSchool*` cases against a lookup table means the suite was written from the same convention, not that the field is reachable. Say so in the summary, list the case IDs that need retiring, and ship the correct contract — do **not** synthesize the field to make the case list line up.
...
0. **🛑 FIRST — is this a GLOBAL LOOKUP / REFERENCE table? If yes, STOP: `ConfigSchoolId` is omitted and you are done (ship `full`).** Do not walk FKs, do not search for a junction, do not run the fan-out pattern. Signals, any one of which should make you check the rest:
   - The table lives in the **`ref.` schema**, or the catalog describes it as a *"lookup"* / *"reference"* / *"type"* table (e.g. *"Application-status lookup (e.g. started, submitted, accepted)"*).
   - Its name ends in `Type` / `Status` / `Code` / `Category`, and its columns are essentially just **an app-assigned PK + a name/description (+ sort order + display flags)**.
   - **It is tiny and static** — a handful to a few dozen rows that are the same for every tenant.
   - **The decisive test:** it carries **neither `SchoolCode` nor `ConfigSchoolID`**, *and* no FK of its own leads to anything that does. Every candidate path is **inward** — you would have to join from the tables that reference *it* (`dbo.OAStudent.appstatusid → …`, `dbo.OAReportSettings.OAReportSettingsTypeId → …`) to ask *"which schools use this value?"*. **An inward join is not a tenancy path.** Other tables pointing at this one is what makes it a lookup, not what makes it tenanted.
   - Contrast with the legitimate cases: `dbo.OAAlumniField` is per-application (`onlineappid → dbo.OA` carries `SchoolCode`) — **outward, one school per row, project it**. `dbo.OARequestInfoQuestion` is per-member (`memberid → dbo.OAInquiryBySchool`) — **outward through a junction, fan it out**. Both *belong to* someone. A status does not belong to a school; it is merely *used by* applications at schools.
   - Log the conclusion in the per-story analysis line as `ConfigSchoolId: NONE (global lookup — <table> has no tenancy key; only inward joins from <consumer tables>)` and see the lookup-table exception above for the required NOTEs and uniqueness assertions.
...
8. **Outcome:**
   - **Global lookup / reference table (step 0)** → omit `ConfigSchoolId` entirely, keep the handler at the plain exemplar shape, add the loud NOTEs + PK-uniqueness assertions, and ship **`full`**. This is a correct implementation, not a gap.
...
> **🛑 Precondition — the entity must BELONG to the owner the junction maps.** This pattern applies when the entity is *owned* by something that covers many schools (`OARequestInfoQuestion` belongs to a member; that member covers many schools). It does **NOT** apply to a global lookup that many tenants merely *use* — see step 0 and the lookup-table exception above. Applying it there fans a lookup out to one row per (value, school) and **breaks PK uniqueness in the response**, which is exactly the PR #3414 defect. Before you reach for this pattern, state in one line **whose** entity this is and **why** that owner is many-schooled; if the honest answer is "it isn't owned by anyone, it's just referenced", you are in step 0, not here.
...
Log a one-line analysis summary per story before coding — include the resolved controller/route/query names, the DTO field count from ViewModels, the **resolved `ConfigSchoolId` join path** (e.g. `OAAlumniField -> OA -> ConfigSchool`, `NONE (omitted)`, or `NONE (global lookup)` per ladder step 0), the **PK shape** (see below), and whether test-case IDs were found (real vs `000`). Add the two per-property audit lines the DTO rules require — `DTO-NAME CHECK:` for every property whose SQL column casing differs from its EF/DTO name, and `DTO-TYPE CHECK:` for every numeric property where ADO and the schema disagree.
...
⚠️ **Do NOT add a synthetic key, a `.Distinct()`, or any other change to the paged query to "resolve" a reported page overlap.** Pagination via Sieve's `GetPagedAsync` is already correct; deviating from the reference PR's shape to satisfy a mis-keyed checker would be a genuine regression. (This bans *overlap-driven* query surgery. It does **not** ban the baseline `.OrderBy` that makes sort snapshots deterministic — see "Deterministic row order" under the API tests.)
...
- **DTO(s)** in `Shared/` — **the field set is dictated by the System Info → ViewModels section** (every listed property, exact name/nullability, incl. `ConfigSchoolId` when listed; **numeric widths mirror the SQL column / EF entity, not the ViewModels type — see the type rule under Step 1**); read DTO with `[ApplySieve]` + `[Sieve(...)]` attributes + a `static Expression<Func<EFModel.<Entity>, <Dto>>> Projection`; write DTO for commands. `[ExcludeFromCodeCoverage]` on DTOs.
...
**Expression-bodied ("lambda") members wherever the body is a single expression.** The user hand-converted generated code to this shape on PR #3273, so emit it directly and save them the edit. The case that keeps coming back is the **API-test helper constructor**:

```csharp
public OAFormSchoolHelper(AdmissionsContext context) => _context = context;   // ✅ generate this

public OAFormSchoolHelper(AdmissionsContext context)                          // ❌ do not generate this shape
{
    _context = context;
}
```

Same rule for single-expression private factories (`private static OA CreateOA(string schoolCode, int portfolioGroupId) => new OA { … };`) and one-line controller actions (`=> await _mediator.Send(request, cancellationToken);`) — the generator already gets those two right; the helper ctor is the gap. **Apply it to every helper in the slice, not most of them** — on PR #3273 three of five new helpers got the lambda ctor and `OAColumnSourceTypeHelper`/`OAMedicalFormFieldHelper` were left block-bodied, which is a diff-level inconsistency a reviewer will read as accidental. Do **not** expression-body a multi-statement method just to satisfy the rule, and (as above) do **not** reformat the pre-existing block-bodied helpers — all ~22 merged ones are block-bodied and are meant to stay untouched.
...
- **API / integration (REQUIRED)** under `Admissions.Tests/ApiTests/Features/<Feature>/`. This tier is NOT optional and NOT "build-only-exempt" — it runs entirely against an **in-memory host** (`: ApiTestParallelizable`, `HttpClientBuilder.GetClient(Client)` → `TestsHelper.Act<PagedResult<<Feature>DtoTm>>(...)`), so it is fully reproducible locally with no live service. It comprises, per feature (copy the reference PR's set):
  ...
  - `ApiTests/Features/<Feature>/Shared/<Feature>DtoTm.cs` — the API-test response model. **Property types must match the DTO exactly** (same `byte?`/`short?`/`int?` widths — see the DTO type rule in Step 1).
  - `ApiTests/Helpers/<Feature>Helper.cs` — the seeder (`GenerateAsync(...)`, seeds a `ConfigSchool` + rows). **Expression-bodied ctor** (`public <Feature>Helper(AdmissionsContext context) => _context = context;`) — see the lambda-members rule above.
  - `ApiTests/Shared/Faker/<Feature>EndpointFakerV1.cs` — the Bogus faker, deterministic via `UseSeed(ApiTestConstants.BogusFakerSeedId)`. Casts must use the DTO/entity's real type (`(byte)f.Random.Number(1, 200)`, `allowNulls ? (byte?)null : …`).
...
**⚠️ A scrubbed field also hides DUPLICATE ROWS — always assert PK uniqueness and row count in `BasicRetrieval`.** Scrubbing renders every row's value as `{Scrubbed}`, so two rows that differ *only* in the scrubbed field serialize identically and a fanned-out response reads as a plausible list. In PR #3414 the `OAStatus` `BasicRetrieval` snapshot was committed with **8 rows for 6 seeded statuses — `Id: 20000` and `Id: 20001` each present twice** — and passed review, because `ConfigSchoolId` was scrubbed and the duplicates looked like ordinary rows. Add to `BasicRetrieval` for every list endpoint, regardless of tenancy shape:
```csharp
responseDto.Results.Should().HaveCount(testData.<Entity>List.Count);   // no unexpected fan-out
responseDto.Results.Select(x => x.Id).Should().OnlyHaveUniqueItems();  // PK unique in the response
```
For a fan-out endpoint (one row per entity × school) the count is `entities × schools` and uniqueness moves to the **pair** — assert `Select(x => new { x.Id, x.ConfigSchoolId }).Should().OnlyHaveUniqueItems()` and say in a NOTE why the bare PK repeats. If you cannot state the expected row count, you do not yet understand the query's shape.
...
- **No `ConfigSchoolId` path**, **or a global lookup table (ladder step 0)** → the DTO omits `ConfigSchoolId` (see the ViewModels rule), so cases **9–15** cannot be produced: skip them, don't fake snapshots. For a lookup table this is expected, not a shortfall — name the ADO case IDs that need retiring in the summary, and make sure `Filter` asserts `HaveCount(1)` on the PK so the contract those cases were groping at is still covered.
...
##### 🔁 Deterministic row order — decide this BEFORE you write the sort tests, not after they fail
A `Verify` snapshot pins the **exact row order**. `ConfigSchoolId` (and most low-cardinality seeded columns) has only **two** distinct values across the seed, so `Sorts=ConfigSchoolId` leaves large groups of tied rows whose order is decided by the query plan — the snapshot then passes locally and flakes on CI. Runs keep rediscovering this the expensive way (2026-08-12 among them): write the tests → the first API run fails on an order mismatch → go read the committed exemplars → patch handlers and tests → rebuild → regenerate every snapshot. Do it up front instead:

1. **Handler:** append a baseline `.OrderBy(dto => dto.<stable unique key>)` after `.Select(<Dto>.Projection)` and before `GetPagedAsync(...)` — the PK (`Id`, or the composite PK's columns in order when there is no surrogate `Id`). Sieve's own `Sorts` still wins; this only breaks ties. **Confirm the exact shape against the committed in-tree exemplars** (`OAReligionField`, `OAInfoQuestionMember`, `OARequestInfoQuestion` GET handlers all carry it) and say in the summary that you followed them, since the newest reference PR may not.
2. **Sort tests:** append the same key as a secondary sort term in the request (the exemplars' `WithTieBreaker(...)` helper), so the expected order is pinned on both sides.
3. **Guard the helper on the sort term with any leading `-` stripped**, not on the raw string. `OAReligionField`'s version tests `sorts.EndsWith("Id")`, which also matches `-ConfigSchoolId` and so silently skips the tie-breaker on the descending case — the very column it was added for. Copy the intent, not that bug, and flag the exemplar for follow-up in the summary.

If the DTO genuinely has no stable unique key to order by, say so in the summary rather than inventing one (see the composite-PK rule in Step 1).
...
#### ⚠️ Known environmental blocker — NuGet restore 401 against the internal feeds (do NOT rediscover it)
If the build fails with `NU1301: Unable to load the service index for source https://pkgs.dev.azure.com/...` or a `401`, **stop debugging it** — this is a machine-credential problem, not your code, and the 2026-08-11 run burned ~8 minutes and a large share of its cost re-deriving the following. Take this path directly and do not iterate past it:

1. **`ADO_PAT` will not work.** The PAT in `{{ENV_FILE}}` has Work-Items scope, not **Packaging**, so both `renweb/RWInternalFeed` and `nbsdev/NbsDevPackages` return 401. Don't build a PAT-authenticated `nuget.config`.
2. **An AAD token authenticates `renweb` only.** `az account get-access-token --resource 499b84ac-1321-427f-aa17-267ca6975798` yields a token that works for `renweb/RWInternalFeed` but **not** `nbsdev/NbsDevPackages`. Do **not** run `az login` (non-interactive task — it blocks).
3. **`nbsdev`-only packages are a hard stop.** `Nbs.Framework.*` (e.g. `Nbs.Framework.HttpClient.Configuration >= 7.1.10`) lives solely on `nbsdev` and is pulled in transitively by `Admissions.Managers` / `Admissions.Infrastructure` / `Admissions.Service.Client`, so `Admissions.Service` and `Admissions.Tests` **cannot** restore. The local `~/.nuget/packages` cache does not hold the required version. Offline/fallback-folder restores will not rescue this — try each of these **at most once**.
4. **Prove it is pre-existing, then salvage:** `git diff --name-only origin/main` must show **zero** `.csproj` / `.props` / `nuget.config` changes from you. Then build the projects that *do* restore — `dotnet build Admissions.Domain` and `dotnet build Admissions.Accessors` — which compiles and validates every EF/`AdmissionsContext` change.
5. **Report and stop.** Emit `BUILD: FAIL` with `(environmental — NuGet restore, not a compile error in generated code)`, mark every story `scaffold`, state plainly that the feature slices and tests are **unverified** and that no `*.verified.txt` snapshots or coverage numbers exist, and let Step 5.1 gate #2 skip all associations. Do **not** hand-write snapshots or claim coverage you could not measure.
6. **Secret hygiene:** if you wrote a temp `nuget.config` containing an access token, write it under `$env:TEMP`, never into the repo, and **delete it before you finish**.
...
If the shortfall is because `ConfigSchoolId` was omitted, treat that as a **red flag on the omission itself**, not a fact to report and move on from: a suite full of `ConfigSchool*` cases is the PO telling you the field is expected. Re-run step 5/6 of the join-path ladder (junction-table search) before accepting the gap. **Unless ladder step 0 says the table is a global lookup** — then the shortfall is correct and expected: the suite was written from the convention, not from the schema. Report it as *"N cases retired — <table> is a global lookup, ConfigSchoolId is not a property of it"*, list the case IDs, and ship `full`.
