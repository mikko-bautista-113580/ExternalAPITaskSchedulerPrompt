...
4. **Accept a path only if it is a real FK AND single-valued** (many-to-one at every hop). A hop that is one-to-many or ambiguous is **not** a usable path — you cannot pick a row. Worked counter-example from the catalog: `dbo.OARequestInfo` is PK `requestid` with only an (indexed, non-FK) `memberid`; `dbo.OAMember` is PK `memberid` with **no `ConfigSchool`/`SchoolCode` link at all**, and `memberid → OA` is one-to-many — so there is genuinely no path, which is why `ConfigSchoolId` was omitted there rather than zero-filled.
5. **Confirm against the story's `sis-sql-schema` link** before writing EF (the live DDL wins over this snapshot), and confirm the navigation/entity state in `Admissions.Accessors/EFModel/`.
6. **Outcome:**
   - **Path found** → project it null-safe through the chain (`e.OA != null && e.OA.ConfigSchool != null ? … : (short)0`), adding only the nullable navigation the EF gate permits.
   - **No path found (proven via steps 1–5)** → apply the omission rule below: drop the property + projection + Sieve attribute, leave the loud `// NOTE(AB#<id>)`, and mark the story `scaffold`.
